...
Future<SharedPreferencesLocalStore> _storeWith(Map<String, Object> prefs) {
  ...
  final encoded = prefs.map((k, v) => MapEntry(k, v is List ? jsonEncode(v) : v));
  SharedPreferences.setMockInitialValues(encoded);
  return SharedPreferences.getInstance()
      .then((p) => SharedPreferencesLocalStore(p));
}
...
void main() {
  ...
  group('v2 迁移', () {
    test('v1 存在且 v2 缺失时 list 读到 submission + revision 0，save 后写 v2 并保留 v1', () async {
