...
class AiPreparationPersonalizer implements PreparationPersonalizer {
  ...
  String _buildUserMessage(PreparationPersonalizationRequest req) {
    ...
    return '【竞赛】${jsonEncode({
          'id': req.competition.id,
          'name': req.competition.name,
          'category': req.competition.category,
          'rules_summary': req.competition.rulesSummary.toJson(),
        })}\n'
        '【目标日期】${req.targetDate.toIso8601String()}\n'
...
你是 SchoNavi 的备赛计划个性化助手。根据【竞赛】、【目标日期】、【每周投入】、【经验等级】、【合法阶段 key】与可选的【学生档案】，为每个合法阶段生成可选任务建议与个性化建议。
规则：
...
6. 仅输出一个 JSON 对象，不要 Markdown、不要多余文字。
输出格式：
