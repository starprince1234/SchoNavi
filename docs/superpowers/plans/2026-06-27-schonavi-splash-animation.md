...
**修正后的接口（Notifier 持 Ticker 引用，skip/markNavigated 落实）：**
- `class SplashController extends Notifier<SplashState>`：
  - 私有字段 `AnimationController? _ticker;` 与 `bool _navigated = false;`。
  - `void attach(AnimationController ticker)` — 页面 initState 调用：保存引用并 `ticker.addListener(() => setProgress(ticker.value))`。
  - `void setProgress(double value)` — 由 Ticker listener 推送；`if (_navigated) return;` 防导航后残余帧；value≥1.0 置完成态。
  - `void skip()` — 跳过动画：`_ticker.value = 1.0`（listener 触发 setProgress(1.0)→完成）；无 ticker 兜底直接置完成。
  - `void markNavigated()` — 页面 fade-out 导航后调用：`_navigated = true; _ticker?.stop();`，后续 setProgress 被忽略。
- 页面职责：`initState` 建 Ticker 并 `notifier.attach(_ticker)`、`forward()`；`_skip()` 调 `notifier.skip()`；`_onFadeOutEnd` 调 `notifier.markNavigated()` + 本地 `_navigated` flag 防 `go()` 重复调用。

> **设计取舍**：Notifier 无 vsync 故不创建 Ticker，但通过 `attach` 持有页面提供的 Ticker 引用，使 `skip`/`markNavigated` 落实为真实行为并被调用。纯逻辑单测覆盖 `setProgress`/`skip`/`markNavigated` 的状态机（无需真 Ticker）；`attach` 的 Ticker 驱动路径由 Task 3 page widget 测试覆盖。
...
**行为：**
- `initState` 创建 `SingleTickerProviderStateMixin` 的 `AnimationController(duration: 1800ms)`，调 `ref.read(splashControllerProvider.notifier).attach(_ticker)` 挂监听，再 `_ticker.forward()` 启动。
- `build`：`Stack` → `CoolScaffoldBackground` + 居中列（logo CustomPaint size=72 + 字标）。字标用 `Opacity` + `Transform.translate`，interval [0.75, 1.0]（progress 派生 wordmarkT）。
- 整页 `GestureDetector.onTap` → `ref.read(splashControllerProvider.notifier).skip()`。
- `isCompleted` 变 true → 外层 `AnimatedOpacity(opacity: 0, duration: 200ms)`，`onEnd`（仅 isCompleted 时）调 `_onFadeOutEnd`：本地 `_navigated` flag 防重 + `ref.read(splashControllerProvider.notifier).markNavigated()` + `ref.read(routerProvider).go('/home')`。
